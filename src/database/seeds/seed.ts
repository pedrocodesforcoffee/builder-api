...
async function seed() {
  ...
  try {
    ...
    for (let i = 0; i < seedOrganizations.length; i++) {
      ...
      // Index 0: admin (added to all orgs as ORG_ADMIN for testing)
      // Index 1,2,7: Acme Construction
      // Index 3,4,8: Summit Builders
      // Index 5,6,9: Elite Properties
      const memberMappings = [
        { userIndex: 0, role: OrganizationRole.ORG_ADMIN }, // System Admin - added for testing
      ...
      if (i === 1) {
        // Summit Builders
        memberMappings[1] = { userIndex: 3, role: OrganizationRole.OWNER };
        memberMappings[2] = { userIndex: 4, role: OrganizationRole.ORG_ADMIN };
        memberMappings[3] = { userIndex: 8, role: OrganizationRole.ORG_MEMBER };
      } else if (i === 2) {
        // Elite Properties
        memberMappings[1] = { userIndex: 5, role: OrganizationRole.OWNER };
        memberMappings[2] = { userIndex: 6, role: OrganizationRole.ORG_ADMIN };
        memberMappings[3] = { userIndex: 9, role: OrganizationRole.ORG_MEMBER };
      }
