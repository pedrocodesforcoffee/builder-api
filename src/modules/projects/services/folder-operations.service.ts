...
export class FolderOperationsService {
  private readonly logger = new Logger(FolderOperationsService.name);
  ...
  async moveFolder(
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      this.logger.log(`[moveFolder] BEFORE SAVE - Folder ${folder.id} (${folder.name}) parentId: ${folder.parentId}`);
      await manager.save(ProjectFolder, folder);
      this.logger.log(`[moveFolder] AFTER SAVE - Folder ${folder.id} (${folder.name}) parentId: ${folder.parentId}`);
      ...
      this.logger.log(`[moveFolder] TRANSACTION COMPLETE - Folder ${folder.id} (${folder.name}) parentId: ${folder.parentId}`);
      return folder;
    });
  }
  ...
  private async copyFolderRecursive(
    ...
    // This would require integration with Document Management System
