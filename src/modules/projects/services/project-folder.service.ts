...
export class ProjectFolderService {
  ...
  async findAll(
    ...
    this.logger.log(`[findAll] Retrieved ${folders.length} folders for project ${projectId}`);
    this.logger.log(`[findAll] Folder parentIds:`, folders.map(f => ({ id: f.id, name: f.name, parentId: f.parentId })));
    return folders;
  }
  ...
  private buildFolderTree(
    ...
    this.logger.log(`[buildFolderTree] Building tree for parentId=${parentId}, found ${children.length} children`);
    this.logger.log(`[buildFolderTree] Children details:`, children.map(f => ({ id: f.id, name: f.name, parentId: f.parentId })));
   ...
   *
   * Helper method for ensuring folder structure exists.
   * If folder doesn't exist, creates it with default settings.
   *
   * @private
   * @param projectId - Project ID
   ...
   * @returns Found or created folder
   */
  private async findOrCreateFolder(
    ...
    if (existing) {
      this.logger.log(`[findOrCreateFolder] Found existing folder: ${name}`);
      return existing;
    }
    ...
    const savedFolder = await this.folderRepository.save(folder);
    this.logger.log(`[findOrCreateFolder] Created new folder: ${name}`);

    return savedFolder;
  }

  /**
   * Ensure commitment folder structure exists
   *
   * Creates the folder hierarchy for a commitment:
   * /Financials/Commitments/{Title}/{Number - Vendor}
   *
   * @param projectId - Project ID
   * @param commitmentId - Commitment ID
   * @param commitmentTitle - Commitment title
   * @param commitmentNumber - Commitment number (e.g., "SC-001")
   * @param vendorName - Vendor name
   * @returns Commitment-specific folder
   */
  async ensureCommitmentFolderStructure(
    projectId: string,
    commitmentId: string,
    ...
    this.logger.log(
      `[ensureCommitmentFolderStructure] Creating folder structure for commitment ${commitmentNumber}`,
    );

    // 1. Find or create: /Financials
    const financialsFolder = await this.findOrCreateFolder(
      projectId,
      null,
      'Financials',
    ...
    // 2. Find or create: /Financials/Commitments
    const commitmentsFolder = await this.findOrCreateFolder(
      projectId,
      financialsFolder.id,
      'Commitments',
      FolderType.GENERAL,
    ...
    // 3. Find or create: /Financials/Commitments/{Title}
    const titleFolder = await this.findOrCreateFolder(
      projectId,
      commitmentsFolder.id,
      commitmentTitle,
      FolderType.GENERAL,
    );

    // 4. Create: /Financials/Commitments/{Title}/{Number - Vendor}
    // Always create a new folder for each commitment (even if duplicate names)
    const folderName = `${commitmentNumber} - ${vendorName}`;

    const folder = new ProjectFolder();
    folder.projectId = projectId;
    folder.parentId = titleFolder.id;
    folder.name = folderName;
    folder.folderType = FolderType.GENERAL;
    folder.inheritPermissions = true;
    folder.isPublic = false;
    folder.permissions = [];
    folder.tags = [];
    ...
    // Calculate path and level
    folder.level = this.validationService.calculateLevel(titleFolder.level);
    folder.path = this.validationService.calculatePath(folder, titleFolder);

    const savedFolder = await this.folderRepository.save(folder);
    this.logger.log(
      `[ensureCommitmentFolderStructure] Created commitment folder: ${folderName}`,
    );
  ...
  /**
   * Get commitment folder by commitment ID
   *
   * Looks up the folder associated with a commitment using customFields.
   *
   * @param commitmentId - Commitment ID
   * @returns Commitment folder
   * @throws NotFoundException if folder not found
   */
  async getCommitmentFolder(commitmentId: string): Promise<ProjectFolder> {
    const folder = await this.folderRepository
      .createQueryBuilder('folder')
      .where("folder.customFields->>'commitmentId' = :commitmentId", {
        commitmentId,
      })
      .getOne();
    ...
    if (!folder) {
      throw new NotFoundException(
        `Folder not found for commitment ${commitmentId}`,
      );
    }
