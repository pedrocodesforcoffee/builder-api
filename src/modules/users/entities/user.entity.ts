...
export class User {
  ...
  /**
   * Get the user's full name (alias for frontend compatibility)
   * Exposed in JSON serialization
   */
  @Expose()
  get name(): string {
    return `${this.firstName} ${this.lastName}`;
   ...
   * Also ensures the 'name' getter is included in serialization
   */
  toJSON(): Partial<User> & { name: string } {
